...
def aes_encoder(block, key):
    ...
    return string_to_bits(str(ecb.encrypt(block)))
...
def cipher_block_chaining(plaintext, key, init_vec, block_size, block_enc):
    ...
    for i in range(len(plaintext)//block_size + 1):
        ...
        if start >= len(plaintext):
            break
        end = min(len(plaintext), start+block_size)
        encryption_input = xor(plaintext[start:end], xor_input)
        cipher_block = block_enc(encryption_input, key)
        xor_input = cipher_block
